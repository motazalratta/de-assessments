...
class AddTimestamps(beam.DoFn):
    def process(self, element, publish_time=beam.DoFn.TimestampParam):
        ...
        try:
            ...
            # AUTO will be considered as current time
...
def run(argv):
    ...
    with beam.Pipeline(options=pipeline_options) as pipeline:
        ...
        normalized_records_result = (
                records | beam.ParDo(Normalization()).with_outputs(Normalization.BAD_TAG, Normalization.GOOD_TAG))

        normalized_bad_records = (normalized_records_result[Normalization.BAD_TAG]
                                  | "NormalizedWriteDeadLetterToBQ" >> WriteDeadLetterToBQ(options,
                                                                                           "Couldn't do the normaliztion")
                                  )
